...
from agent_world.windsurf_brain import (
    AcpProtocolError,
    AcpTurnResult,
    WindsurfBrain,
    _AcpClient,
    _subscription_environment,
    build_windsurf_prompt,
    parse_windsurf_model_catalog,
    resolve_windsurf_model,
    run_windsurf_acp_turn,
)
...
def _turn(
    *,
    ...
    session_id: str = "windsurf-session",
...
class WindsurfBrainTests(unittest.TestCase):
    def test_preflight_accepts_saved_login_and_account_model(self) -> None:
        ...
        with patch(
            "agent_world.windsurf_brain.subprocess.run",
            side_effect=[status, models],
        ) as run:
            brain = WindsurfBrain(executable="devin", model="swe-1-6-fast")
    ...
    def test_preflight_rejects_missing_login(self) -> None:
        ...
        with patch("agent_world.windsurf_brain.subprocess.run", return_value=status):
            error = WindsurfBrain(executable="devin").preflight()
        self.assertIn("run `devin auth login`", error or "")

    def test_model_catalog_and_resolution_are_account_scoped(self) -> None:
        available = parse_windsurf_model_catalog(
            {
                "models": [
        ...
        self.assertEqual(
            resolve_windsurf_model("swe-1-6-fast", available),
            "swe-1-6-fast",
        )
        self.assertEqual(resolve_windsurf_model("adaptive", available), "adaptive")
        self.assertIsNone(resolve_windsurf_model("not-on-account", available))
    ...
    def test_decide_records_acp_provenance_usage_and_strips_provider_keys(self) -> None:
        ...
        try:
            with patch(
                "agent_world.windsurf_brain.run_windsurf_acp_turn",
        ...
        self.assertEqual(record["windsurf_session_id"], "windsurf-session")
    ...
    def test_malformed_model_output_is_preserved_with_failure_metadata(self) -> None:
        ...
        with patch(
            ...
            brain = WindsurfBrain(executable="devin")
            decision = brain.decide(
        ...
        self.assertTrue(
            decision.intent.startswith("Windsurf model output contract failed:")
        )
        record = brain.runtime.usage_records()[0]
    ...
    def test_timeout_records_failure_and_opens_provider_scope_circuit(self) -> None:
        with patch(
            ...
            brain = WindsurfBrain(executable="devin", timeout_seconds=3)
        ...
        self.assertTrue(first.intent.startswith("Windsurf provider unavailable:"))
        self.assertEqual(first.intent, second.intent)
        record = brain.runtime.usage_records()[0]
        self.assertEqual(record["decision_failure_origin"], "ambiguous_boundary")
        self.assertEqual(record["windsurf_stop_reason"], "error")
    ...
    def test_tool_call_update_is_rejected_as_a_boundary_failure(self) -> None:
        ...
        self.assertTrue(decision.intent.startswith("Windsurf boundary failed:"))
        self.assertEqual(
            brain.runtime.usage_records()[0]["windsurf_tool_call_count"],
            1,
        )

    def test_quota_and_auth_errors_fail_safely(self) -> None:
        for detail, prefix in (
            ("Usage limit reached", "Windsurf quota unavailable:"),
            ("Not logged in", "Windsurf provider unavailable:"),
        ):
    ...
    def test_acp_turn_negotiates_session_and_collects_streamed_usage(self) -> None:
        ...
        with patch("agent_world.windsurf_brain._AcpClient", FakeClient):
            result = run_windsurf_acp_turn(
                command=["devin", "acp"],
                cwd="/tmp",
                prompt="choose",
    ...
    def test_environment_and_prompt_are_isolated(self) -> None:
        ...
        prompt = build_windsurf_prompt("RULEBOOK", '{"tick":1}')
