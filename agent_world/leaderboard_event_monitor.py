...
def worker(root, event_dir):
    ...
    prompt = ("You are the low-effort Astra event worker for the existing Run Monitoring worklist. "
              ...
              "using repository benchmark guidance; preserve checkpoints and source provenance. "
              "For output-contract failures, reconstruct the actual request from the pinned source and "
              "compare recorded prompt hashes where available. Verify the model received the full required "
              "contract before attributing malformed responses to instruction-following behavior. A parser "
              "rejecting an output does not prove the connector supplied the schema. Record any omitted "
              "requirements as a harness condition; do not silently change prompts, waive startup gates, "
              "or relabel a corrected diagnostic as the original benchmark. "
              "An authentication label is not proof that the user must sign in. Verify the native harness "
              "under the same OS user before declaring an external blocker. For Grok, run grok models "
              "in the WSL runtime using the saved subscription login; this often succeeds without a new "
              "sign-in. If it lists the requested model, resume the existing managed checkpoint. "
              "Do not ask for grok login based only on a historical preflight error. A resume command "
