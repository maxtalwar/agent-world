...
**Fable 5 exhausted a session allowance after 134 decisions in about eight minutes, while comparable v8.1 decisions used only 7.1% more input and 0.9% more output than v6.** The historical study recorded Max; the current CLI initially reported Pro. **Follow-up correction:** the user had paid for a Max month and scheduled a next-cycle Pro downgrade. The CLI was already current at 2.1.263; completing a fresh subscription sign-in changed its reported tier to Max. This confirms stale CLI metadata, not a verified loss of paid entitlement. V8.1 also overlaps two four-worker cells, whereas the historical seeds ran separately. Cache reads cover 67.7% of current input, and 140 started requests resolve to 134 distinct successful decisions plus six quota refusals, with no retry storm. Both worlds then froze until the stated reset. Whether stale metadata affected server quota enforcement, and the exact attribution of the user's reported 8% weekly usage, remain unverified. This is an operational/entitlement finding, not a change in model capability. See [counts, source artifacts, plan evidence, and caveats](fable-v81-usage-investigation.md).
