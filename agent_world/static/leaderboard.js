...
function studyMarkup(run, grouped=false) {
  ...
  const reviewedEvidence=(affected||provenanceReview)&&request?.monitor_reviewed&&request?.monitor_resolution==='evidence_decision';
  ...
  const status=executionReview?'Continuation needs approval':finishedExperiment?'Completed':run.ranked?'Ranked':repairing?'Repair in progress':externalBlocker?'Needs attention':affected&&reviewedEvidence?'Continuation needs a decision':affected?'Run paused':provenanceReview?(agentWorking?'Review in progress':reviewedEvidence?'Review needs a decision':'Review pending'):quota?'Quota paused':'In study';
  ...
  if(request?.monitor_resolution_reason&&!(affected&&reviewedEvidence))diagnostics.push(request.monitor_resolution_reason);
  ...
  return '<article class="study"><div class="study-head"><span>'+esc(run.model)+'</span><span class="study-status">'+(grouped?'':status)+'</span></div>'+
    ...
    (affected&&reviewedEvidence&&request?.monitor_resolution_reason?'<p class="study-note">'+esc(request.monitor_resolution_reason)+'</p>':'')+
