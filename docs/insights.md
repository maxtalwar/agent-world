...
## 2026-09-22 - Interleaving removed the apparent Astra cache advantage

**Astra and Sol had identical cache-hit class counts when their static content and call order were controlled, and both degraded late in the same pass.** Two counterbalanced seed-11 first-tick experiments made 40 decisions plus four template initializations on CLI 0.156.0 at medium effort. Twenty observation hashes matched across swapped model assignments and all static rulebook hashes matched. Each model had 17 near-template reads, two partial reads, and one miss; the two misses occurred consecutively about 14 seconds apart, despite same-model idle gaps below 31 seconds. This contradicts a persistent Astra-over-Sol cache advantage, not the earlier measurements. Time-varying cache availability/routing is plausible, but not proven, and Sol's earlier rulebook differed. Transferring the new hit frequencies to the original prompt/output budgets projects about 30% Astra and 22% Sol savings versus recorded benchmark costs, with substantial pass-to-pass variation. The same review estimates conditional 1h-to-5m Claude savings of 22% for Fable 5.1 and 19-20% for Opus 5, while identifying that historical ledgers omitted TTL splits and leaderboard costs already used 5m pricing. [Measurements, limitations, and reproduction](cache-followup-20260922.md).

## 2026-09-22 - A large matched cache saving need not beat a historically warm run

**Native shared-prefix forks cut today's Astra and Sol input costs sharply, but Sol's historical baseline already cached better than the new sample.** In 42 managed calls at medium effort (seed 11, ten matched observations per condition), Astra forks read 14,336 cached tokens on 10/10 decisions versus zero fresh hits; Sol forks had five full-prefix hits, four partial hits, and one miss versus two partial fresh hits. Input cost fell 85.8%/63.9%. Including template cost, historical prompt growth, and fixed historical output budgets projects current-CLI full-run savings of 75.5%/51.2%. Against the actual historical ledgers, however, the projected costs are only 43.2%/2.8% lower: historical Sol cached 14,080 tokens on 371/500 calls. A sensitivity retaining old total prompt budgets makes Sol 13.9% more expensive. This is a harness/cache-routing finding, not a model capability improvement, and ten sequential calls cannot establish sustained or subscription-quota savings. [Full evidence and reproducible analysis](cache-cost-study-20260922.md).
...
## 2026-09-22 - The expensive prompt tokens were the harness's cache policy, not the rulebook

**Half of a Fable 5.1 decision's cost was a cache write of content that is never read.** Per decision, Claude Code sends ~5,800 cached harness tokens, our ~2,100-token rulebook (already cached, byte-identical across all 600 calls), and a ~1,400-token observation. On a subscription Claude Code defaults to a one-hour prompt cache, whose writes bill at 2x input; it writes the observation on every call and, because each decision is a fresh conversation, never reads it back. Cache writes were 48% of per-call cost against 2% for all cached reads. Setting `CLAUDE_CODE_PROMPT_CACHE_TTL=5m` keeps the rulebook cached and drops the write rate to 1.25x, about 17% of run cost, with no change to what the model sees. Codex shows the mirror image: its cache hit boundary sat at exactly 7,936 tokens across every layout tried, so our content is billed at full price there and no connector-side change fixed it. Shrinking the rulebook, the intuitive fix, would have saved about 1% on Claude. Evidence: `runs/managed/web-claude-fable-5-1-22113c5f3d1f/seed-41/run-usage.jsonl`, `runs/managed/gpt-6-astra-v8-revised-20260907/seed-11/run-usage.jsonl`, captured requests and sequential-call cache measurements on Sonnet 5 and Luna, 2026-09-22.
...
## 2026-09-07 — Astra built resilient local cooperation without a formal market

**Astra lost no health after winter in either v8.1 seed, while every other model in the nine-model comparison continued deteriorating; its zero accepted-trade count nevertheless hides a completed paid delivery.** The no-healing seeds 11/41 ended with 20/20 survivors, mean health 81.25, 15 shelters, eight stores, and 106 maintenance actions. In seed 11, agents 1/5/6 jointly completed shelter at tick 15 and storage at tick 24; all three finished at 100 health. In seed 41, agent 5 offered four coins for two stone at tick 35, agent 3 delivered and completed the shelter at tick 36, and payment occurred at tick 37. Local water access and other conditions confound any causal claim that cooperation alone produced the health advantage. This is strong subsistence and practical coordination, not evidence of an elaborate market: no groups or formal accepted trades emerged. See [narrative, comparable scores, caveats, and exact ledger pointers](astra-v81-narrative.md).

## 2026-09-08 — The v6 recipe name did not freeze historical world capabilities

**A new Astra run labeled Participant v6 used a global message board absent from the historical Sol v6 world, so its clean completion alone does not establish historical comparability.** Study `web-gpt-6-astra-1dafc66aaaea`, launch `13f7616e76b21edef0bae6b551c7589d6fe59531`, completed seeds 11/41 with clean integrity, full usage coverage, and frozen gift classification. Its `run.jsonl` files record 243 and 255 `ledger_note` events. Historical Sol seed 11 (`runs/benchmarks/gpt-5-6-sol-participant-v6-provisional-seed11-20260728-195329/seed-11/run-manifest.json`) pins `7bbff8db635a0ea32a40c9bc812ca93c7ab34d65`. Comparing `interface.py`, `rules.py`, and world source between those commits shows added global ledger observations/instructions and organic delivery contracts. The current v6 recipe inherits these capabilities even though its horizon, population, medium effort, survival parameters, and scoring revision still match. `stateless-v3`/`stateless` versus `connector-v3`/`fresh-conversation` are documented aliases, not evidence of a behavior change. The world-capability drift is substantive; its effect on Astra's score is not measured. Preserve this run as completed evidence, but do not describe it as identical to the historical Sol condition. The release-projection safeguard exposed this issue; it did not cause it. **Follow-up:** the owner requested historical v6 restoration and named the message-board treatment v6.1. Replaying both historical Sol seeds on the restored engine and both Astra seeds on the v6.1 engine exactly reproduced endpoint health, survival, inventories and positions. See [restoration and launch safeguards](benchmark-recipe-restoration-20260908.md).

## 2026-09-07 — Fable's fast quota exhaustion was not a token-use explosion

**Fable 5 exhausted a session allowance after 134 decisions in about eight minutes, while comparable v8.1 decisions used only 7.1% more input and 0.9% more output than v6.** The historical study recorded Max; the current CLI initially reported Pro. **Follow-up correction:** the user had paid for a Max month and scheduled a next-cycle Pro downgrade. The CLI was already current at 2.1.263; completing a fresh subscription sign-in changed its reported tier to Max. This confirms stale CLI metadata, not a verified loss of paid entitlement. The authenticated web Usage and Billing pages subsequently confirmed active Max (5x), while the session remained 100% used and both weekly meters remained 8%; the login refresh did not restore usable allowance. V8.1 also overlaps two four-worker cells, whereas the historical seeds ran separately. Cache reads cover 67.7% of current input, and 140 started requests resolve to 134 distinct successful decisions plus six quota refusals, with no retry storm. Both worlds then froze until the stated reset. Whether stale metadata affected server quota enforcement, and the exact attribution of the user's reported 8% weekly usage, remain unverified. This is an operational/entitlement finding, not a change in model capability. See [counts, source artifacts, plan evidence, and caveats](fable-v81-usage-investigation.md).
